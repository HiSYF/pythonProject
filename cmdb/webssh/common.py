...
def ssh3(ip, cmd,seconds=10):
    ...
    try:
        ...
        print(ip)
        # ssh.connect(ip, settings.SSH_PORT,'root','Sdls@#$2021', timeout=seconds)  # 连接远程服务器,超时时间1秒
        aa = ssh.connect(ip,username='root',password='Sdls@#$2021',allow_agent=False,look_for_keys=False)  # 连接远程服务器,超时时间1秒
